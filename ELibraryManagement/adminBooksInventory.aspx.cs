...
{
    public partial class adminBooksInventory : System.Web.UI.Page
    {
        ...
        void updateBookById()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    // Adding genres which can be multiple values.
                    string genres = "";
                    ...
                    {
                        FileUpload1.SaveAs(Server.MapPath("booksinventory/" + filename));
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE book_master_tbl SET book_name=@book_name, genre=@genre," +
                        "author_name=@author_name, publisher_name=@publisher_name, publish_data=@publish_data, " +
                    ...
                    cmd.Parameters.AddWithValue("@publish_data", TextBox3.Text.Trim());
        ...
        void getBookByID()
        {
            try
            {
                ...
                if(dt.Rows.Count >= 1)
                {
                    ...
                    TextBox3.Text = dt.Rows[0]["publish_data"].ToString();
        ...
        void addNewBook()
        {
            try
            {
                ...
                genres = genres.Remove(genres.Length - 1);
                Page.Response.Write("<script>console.log('" + genres + "');</script>");

                // Adding the img link to out db.
                string filepath = "~/booksinventory/books1.png";
                string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
                FileUpload1.SaveAs(Server.MapPath("booksinventory/" + filename));
                filepath = "~/booksinventory/" + filename;
                Page.Response.Write("<script>console.log('" + filepath + "');</script>");
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO book_master_tbl(book_id, book_name, genre, author_name, publisher_name, publish_data," +
                    "language, edition, book_cost, no_of_pages, book_description, actual_stock, current_stock, book_img_link) " +
                    "values(@book_id, @book_name, @genre, @author_name, @publisher_name, @publish_data, @language, @edition, @book_cost," +
